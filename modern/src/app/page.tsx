...
export default function TajwidPage() {
  ...
  const { recordWordMastered, recordVerseMastered } = useUserStats();
  ...
  const { isListening, isProcessing, toggleListening } = useWhisperRecognition(
    (transcript) => {
  ...
  const tajwidLegend = useMemo(
    () => [
      ...
      {
        label: "Idghām (avec ghunnah)",
        className: "tajwid-idgham-with-ghunnah",
      },
      ...
      {
        label: "Madd Munfaṣil (permissible)",
        className: "tajwid-madda-permissible",
      },
      {
        label: "Madd Muttaṣil (necessary)",
        className: "tajwid-madda-necessary",
      },
      {
        label: "Madd Lāzim (compulsory)",
        className: "tajwid-madda-compulsory",
      },
    ],
    [],
  );
      ...
      if (result.isMatch) {
        ...
        if (!completedWords.includes(activeWordIdx)) {
          setCompletedWords((prev) => {
            const next = prev.includes(activeWordIdx)
              ? prev
              : [...prev, activeWordIdx];
            // Record word in Firestore
            recordWordMastered(currentVerse.ref);
            // If all words are completed, record verse
            if (next.length === currentVerse.words.length) {
              recordVerseMastered(currentVerse.ref);
            }
            return next;
          });
          ...
          >
            ...
            <div className="mb-12 text-center">
              ...
              <p className="text-zinc-500 text-sm italic">
                {completedWords.length} of {currentVerse.words.length} words
                mastered
              </p>
            </div>
            ...
            >
              ...
              <div className="flex items-center gap-4 mb-6">
                <div
                  className={`p-4 rounded-full ${isListening ? "bg-emerald-500 shadow-[0_0_30px_rgba(16,185,129,0.5)]" : "bg-white/5 border border-white/10"} transition-all duration-500`}
                >
                  <Mic
                    className={
                      isListening
                ...
                <div>
                  ...
                  {isListening && (
                    <div className="flex items-center gap-1 mt-1">
                      ...
                      <span className="text-[8px] text-emerald-500 uppercase font-bold tracking-tighter">
                        Smart Noise Filter Active
                      </span>
                    </div>
                  )}
                </div>
              ...
              <AnimatePresence mode="wait">
                {isProcessing && (
                  <motion.div
                    ...
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                    className="mb-6 flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-widest bg-blue-500/20 text-blue-500"
                  >
                    <Loader2 size={14} className="animate-spin" />
                    Analyzing Tajwid...
                  </motion.div>
                )}
                {feedback.type !== "none" && !isProcessing && (
                  <motion.div
              ...
              <div className="flex items-start justify-between gap-6 mb-6">
                <div>
                  ...
                  <p className="text-zinc-400 text-sm mt-1">
                    Les couleurs/traits viennent directement des balises Tajwid
                    de Quran.com.
                  </p>
                </div>
                <button
                ...
                {tajwidLegend.map((item) => (
                  ...
                  >
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <div className={`font-amiri text-xl ${item.className}`}>
                        نم
                      </div>
                    </div>
          ...
          >
            ...
            >
              ...
              <p className="text-zinc-400 mb-12">
                Learn together with real-time peer-to-peer audio and sync.
              </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
                ...
                >
                  ...
                  <div className="relative z-10 flex flex-col items-center justify-center p-8">
                    ...
                    <span className="text-[10px] text-zinc-500 group-hover:text-white/70 mt-1 uppercase">
                      Lead Recitation
                    </span>
                  </div>
                </button>
